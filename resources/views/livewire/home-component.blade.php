<main id="main">
    <div class="container">
        ...
        <!--On Sale-->
        @if ($products_on_sale->count() > 0 && $sale->status == 1 && $sale->sale_date > Carbon\Carbon::now())
                    ...
                    @foreach ($products_on_sale as $product_on_sale)
                        <div class="product product-style-2 equal-elem ">
                            <div class="product-thumnail">
                                <a href="{{ route('product.details', ['slug' => $product_on_sale->slug]) }}" title="{{ $product_on_sale->name }}">
                                    <figure><img src="{{ asset('assets/images/products') }}/{{ $product_on_sale->image }}" width="800" height="800" alt="{{ $product_on_sale->name }}"></figure>
                                </a>
                            ...
                            <div class="product-info">
                                <a href="{{ route('product.details', ['slug' => $product_on_sale->slug]) }}" class="product-name"><span>{{ $product_on_sale->name }}</span></a>
                                <div class="wrap-price"><ins><p class="product-price">${{ $product_on_sale->sale_price }}</p></ins> <del><p class="product-price">${{ $product_on_sale->regular_price }}</p></del></div>  
                            </div>
                        </div>
                    @endforeach
                </div>
                                ...
                                @foreach ($latest_products as $latest_product)
                                    <div class="product product-style-2 equal-elem ">
                                        <div class="product-thumnail">
                                            ...
                                            <div class="wrap-btn">
                                                <a href="{{ route('product.details', ['slug' => $latest_product->slug]) }}" class="function-link">quick view</a>
                                            </div>
                                        </div>
                                        <div class="product-info">
                                    ...
                                    @foreach ($category_products as $category_product)
                                        <div class="product product-style-2 equal-elem ">
                                            <div class="product-thumnail">
                                                ...
                                                <div class="wrap-btn">
                                                    <a href="{{ route('product.details', ['slug' => $category_product->slug]) }}" class="function-link">quick view</a>
                                                </div>
                                            </div>
                                            <div class="product-info">
                                                <a href="{{ route('product.details', ['slug' => $category_product->slug]) }}" class="product-name"><span>{{ $category_product->name }}</span></a>
                                                @if ($category_product->sale_price)
                                                    <div class="wrap-price"><ins><p class="product-price">${{ $category_product->sale_price }}</p></ins> <del><p class="product-price">${{ $category_product->regular_price }}</p></del></div>  
                                                @else
                                                    <div class="wrap-price">
                                                        <span class="product-price">
                                                            {{ $category_product->regular_price }}
                                                        </span>
                                                    </div>
                                                @endif
                                            </div>
